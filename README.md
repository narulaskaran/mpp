...
**`/paid/onchain`** — direct on-chain payment via Tempo. Payment received, then the server immediately spends it to call OpenAI via [mpp.dev/services](https://mpp.dev/services) to generate a fresh joke. Remaining balance sweeps to a personal wallet after each request. Use `?testnet=true` for Tempo Moderato testnet/pathUSD or `?testnet=false` for mainnet/USDC.e; without the query param, `TEMPO_TESTNET` remains the default.
